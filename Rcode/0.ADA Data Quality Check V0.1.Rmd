...
# data_sav <- haven::read_sav("Z:/assda/publish/ANU/Archivists/Tina/Lowy Poll/100047 - Lowy Poll 2021/Processing/2643 Life in Australia Wave 47 - final Lowy data file (23042021).sav")
# params$data_sav <- haven::read_sav('../Data/Cohort 2_ADA_wave 11.sav')
...
varLabel <- var_label(params$data_sav)
varName  <- attributes(params$data_sav)$name
...
v_number  <- which (as.numeric(nchar (unique_var_label))%in% c(0:29))   #position of the variable lable that range from 1 to 29
...
# tab1      <- data.frame(matrix(
#   cbind(
#     seri_col, 
#     var_info$label[v_number], 
#     as.numeric(nchar(unique(var_info$label)[v_number])), 
#     su_col, nv_col, nv_col ), 
#   
#   ncol = 6))
...
v_number  <- which (as.numeric(nchar (unique_var_label)) %in% c(81:100))      #lenth between 81 and 100
seri_col <- paste(v_number, varName[v_number], sep = ' :');
...
```{r message = FALSE, warning = FALSE, echo = FALSE, comment=NA, results='asis'} 
v_number  <- which (as.numeric(nchar (unique_var_label)) %in% c(101:150))
seri_col <- paste(v_number, varName[v_number], sep = ' :');
nv_col <- rep('',length(seri_col)); su_col <- rep('KO/SN',length(seri_col))
...
```{r message = FALSE, warning = FALSE, echo = FALSE, comment=NA, results='asis'} 
v_number  <- which (as.numeric(nchar (unique_var_label))> 150)      #varible label length larger than 150
seri_col <- paste(v_number, varName[v_number], sep = ' :');
nv_col <- rep('',length(seri_col)); su_col <- rep('KO/SN',length(seri_col))
