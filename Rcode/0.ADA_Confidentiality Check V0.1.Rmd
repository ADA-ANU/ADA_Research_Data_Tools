...
# data_sav <- haven::read_sav('../Data/SRC2793 ADA Final 04-11-2022.sav')
...
for (i in 1: length (conf_var)) {                                              # For every element in conf_var defined on the top
  posi[i] <- which(varName == conf_var[i])                   # Find the position of corresponding element
}                                                                              # 
...
if (nrow(demographic_identifiable)>0){

  theme_box(flexPrintFun(convert_list_columns_to_character(demographic_identifiable) ))
} else{
  theme_box(flexPrintFun(data.frame(sapply( demographic_identifiable, as.character))))
